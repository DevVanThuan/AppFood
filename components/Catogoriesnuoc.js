...
import { DOUONG, THUCAN } from '../app/FoodImage/FoodData';
import { FontAwesome, FontAwesome6 } from '@expo/vector-icons';
import { TouchableOpacity } from 'react-native';

export default function Catogoriesnuoc() {
    const ngoisao = [];
    for (let i = 0; i < 5; i++) {
        ngoisao.push(
            <FontAwesome
    ...
    return (<View>
        <Text style={{marginLeft:10, fontWeight:'bold', fontSize:20}}>Đồ Uống</Text>
        <FlatList
            data={DOUONG}
            renderItem={(a) => {
                return (
                    ...
                    <TouchableOpacity>
                        <View style={styles.container}>
                            <Image style={styles.img} source={a.item.icon} />
